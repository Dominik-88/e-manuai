...
export default function NewAreaPage() {
  ...
  const form = useForm<AreaFormData>({
    ...
    defaultValues: {
      ...
      okres: undefined,
      plocha_m2: undefined,
      obvod_oploceni_m: undefined,
      gps_latitude: undefined,
      gps_longitude: undefined,
      google_maps_link: '',
      kategorie_travnate_plochy: '',
      poznamky: '',
    },
  });
    ...
    try {
      const { error } = await supabase
        ...
        .insert({
          ...
          plocha_m2: data.plocha_m2 || null,
          obvod_oploceni_m: data.obvod_oploceni_m || null,
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">Nový areál</h1>
          <p className="text-muted-foreground">Přidejte novou lokalitu</p>
        </div>
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <div className="dashboard-widget space-y-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input {...field} className="h-14 text-base" placeholder="např. VDJ Písek – Hradiště" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      ...
                      <SelectContent>
                        {['vodojem', 'úpravna vody', 'čerpací stanice', 'vrt', 'vinice', 'sad', 'park', 'zahrada', 'jiné'].map(t => (
                          <SelectItem key={t} value={t} className="h-12 capitalize">{t}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                    <FormMessage />
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      ...
                      <SelectContent>
                        {(Object.entries(OKRES_NAMES) as [OkresCode, string][]).map(([code, name]) => (
                          <SelectItem key={code} value={code} className="h-12">{name}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                    <FormMessage />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input 
                        type="number" step="0.00001" {...field}
                        onChange={e => field.onChange(e.target.value ? parseFloat(e.target.value) : undefined)}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input {...field} className="h-12" placeholder="https://maps.google.com/..." />
                  </FormControl>
                  <FormDescription>
                    <a href="https://maps.google.com" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline flex items-center gap-1">
                      <MapPin className="h-3 w-3" /> Otevřít Google Maps
                    </a>
                  </FormDescription>
                  <FormMessage />
                </FormItem>
            ...
            <FormField
              control={form.control}
              name="kategorie_travnate_plochy"
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Kategorie travnaté plochy</FormLabel>
                  <FormControl>
                    <Input {...field} className="h-12" placeholder="např. A, B, C" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
