    ...
    try {
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          rezim: data.rezim as any,
          s_mode: data.s_mode || null,
          mth_start: data.mth_start,
          mth_konec: data.mth_konec || null,
          mth_delta: mthDelta,
          plocha_obdelana_m2: data.plocha_obdelana_m2 || null,
          rtk_stav: data.rtk_stav as any,
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <div className="dashboard-widget space-y-4">
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} value={field.value}>
                    ...
                    <SelectContent>
                      {areas?.map(area => (
                        <SelectItem key={area.id} value={area.id} className="h-12">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <FormField
                control={form.control}
                name="datum_cas_start"
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Začátek *</FormLabel>
                    <FormControl>
                      <Input type="datetime-local" {...field} className="h-14 text-base" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Provozní režim *</FormLabel>
                  <FormControl>
                    <RadioGroup
                    ...
                    >
                      {[
                        ...
                        { value: 'poloautonomní', label: 'Poloauto' },
                        { value: 'autonomní', label: 'Autonomní' },
                      ].map(mode => (
                        <Label
                          key={mode.value}
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={v => field.onChange(parseInt(v))} value={field.value?.toString() || ''}>
                      ...
                      <SelectContent>
                        {Object.entries(S_MODE_NAMES).map(([key, name]) => (
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    {mthKonec && mthStart && mthKonec > mthStart && (
                      <FormDescription className="font-mono">
                        Δ {(mthKonec - mthStart).toFixed(1)} mth
                      </FormDescription>
                    )}
                    <FormMessage />
                  </FormItem>
